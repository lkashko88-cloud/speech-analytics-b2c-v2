  ...
  document.getElementById('real-conclusions').innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      ${belowNorm.slice(0, 3).map(c => {
        const examples = {
          'Контакт': [
            { bad: 'Алло... да... здравствуйте', good: 'Здравствуйте, меня зовут Алексей, компания Дом.ру, слушаю вас', note: 'Нет представления, имени, компании — клиент не понимает, с кем говорит' },
            { bad: 'Угу, так... сейчас подождите', good: 'Назовите, пожалуйста, адрес — я проверю информацию по вашему договору', note: 'Междометия вместо вежливых фраз создают ощущение безразличия' },
          ],
          'Потребности': [
            { bad: 'Адрес скажите... так, смотрю... ну вот у вас тариф такой', good: 'Скажите, сколько устройств подключаете к интернету? Что для вас важнее — скорость или стабильность? Смотрите ли ТВ?', note: 'Оператор сразу переходит к решению, не выяснив потребности' },
            { bad: 'Улучшаем качество услуг... пользуетесь интернетом, верно?', good: 'Хочу убедиться, что ваш тариф подходит под ваши задачи. Расскажите, как используете интернет — работа, фильмы, игры?', note: 'Закрытый вопрос вместо открытого — клиент отвечает «да/нет» и разговор буксует' },
          ],
          'Презентация': [
            { bad: 'Ну вот, у нас есть тариф... ээ... там интернет и каналы', good: 'Учитывая, что у вас 3 устройства — подойдёт тариф «Кино и развлечения»: скорость 300 Мбит, 200 каналов, за 599 руб./мес.', note: 'Нет привязки к потребности клиента, нет конкретных выгод и цифр' },
            { bad: 'Сейчас вас переведу на специалиста по подключению', good: 'Давайте я сразу оформлю заявку — мастер приедет завтра, подключение бесплатное. Удобно в первой или второй половине дня?', note: 'Оператор «сбрасывает» клиента вместо того, чтобы довести до результата' },
          ],
          'Возражения': [
            { bad: 'Ну... адрес скажите, я посмотрю', good: 'Понимаю вашу ситуацию. Давайте разберёмся — расскажите, что именно не устраивает, и я предложу решение', note: 'Клиент хочет отключиться, а оператор игнорирует возражение и просит адрес' },
            { bad: 'Сейчас переведу на старшего специалиста, трубку не кладите', good: 'Я вижу проблему и могу помочь прямо сейчас. Мы можем изменить тариф на более выгодный — хотите, расскажу подробнее?', note: 'Перевод звонка = потеря клиента. Нужно удерживать и решать возражение самому' },
          ],
          'Завершение': [
            { bad: 'Ну ладно... до свидания', good: 'Итак, мы с вами решили вопрос по тарифу. Заявка оформлена, мастер приедет 15-го. Есть ещё вопросы? Спасибо за звонок!', note: 'Нет подведения итогов, нет фиксации договорённостей — клиент забудет детали' },
            { bad: 'Хорошо, хорошо... до свидания', good: 'Подведу итог: мы подключили тариф «Игровой» за 799 руб. Если возникнут вопросы — звоните, мы работаем 24/7. Хорошего дня!', note: 'Двойное «хорошо» без содержания — упущен шанс закрепить результат' },
          ],
          'Модули': [
            { bad: 'Соединяю вас со специалистом... не отключайтесь', good: 'Сейчас переведу вас на отдел оформления. Коллега уже видит вашу заявку — повторно объяснять не придётся. Не кладите трубку', note: 'Нет объяснения, зачем переводят и что будет дальше — клиент теряет доверие' },
            { bad: '(звонок обрывается без прощания)', good: 'Рада была помочь! Напоминаю: если скорость упадёт — перезагрузите роутер. Приятного вечера, до свидания!', note: 'Нет завершающего модуля — потеряна возможность оставить приятное впечатление' },
          ],
          'Слушание': [
            { bad: 'Угу... угу... угу... так...', good: 'Правильно ли я понимаю — у вас не работает интернет с утра и вы уже перезагружали роутер?', note: 'Междометия — не слушание. Нужно перефразировать, чтобы клиент видел, что его поняли' },
          ],
          'Тон': [
            { bad: 'Слышь... ну чё... ага...', good: 'Добрый день! Подскажите, пожалуйста, чем могу помочь?', note: 'Разговорная речь и сленг — грубое нарушение стандарта. Клиент чувствует неуважение' },
          ],
          'Активность': [
            { bad: '(длинные паузы, односложные ответы)', good: 'Пока загружается информация — подскажите, вы довольны скоростью интернета? Может, подобрать тариф быстрее?', note: 'Паузы — упущенное время. Оператор должен вести диалог и заполнять паузы полезным контентом' },
          ],
          'Грамотность': [
            { bad: 'Ну типа вам щас это... подключим короче', good: 'Сейчас оформлю подключение. Услуга будет активирована в течение часа', note: 'Слова-паразиты подрывают доверие к компетентности оператора' },
          ],
        };
        const exs = examples[c.name] || [{ bad: '—', good: '—', note: '' }];
        return exs.map(ex => `
          <div style="padding:14px 16px;background:var(--bg-card);border:1px solid var(--border-light);border-radius:10px;">
            <div style="font-weight:600;font-size:13px;color:var(--text-secondary);margin-bottom:8px;">${c.name} · оценка ${c.avg.toFixed(1)}</div>
            <div style="display:grid;grid-template-columns:1fr auto 1fr;gap:10px;align-items:start;">
              <div style="padding:10px 12px;background:#fef2f2;border-left:3px solid #ef4444;border-radius:0 8px 8px 0;">
                <div style="font-size:11px;font-weight:600;color:#ef4444;margin-bottom:4px;text-transform:uppercase;">Как в звонке</div>
                <div style="font-size:13px;font-style:italic;">«${ex.bad}»</div>
              </div>
              <div style="padding-top:12px;font-size:20px;color:var(--text-muted);">→</div>
              <div style="padding:10px 12px;background:#f0fdf4;border-left:3px solid #10b981;border-radius:0 8px 8px 0;">
                <div style="font-size:11px;font-weight:600;color:#10b981;margin-bottom:4px;text-transform:uppercase;">Как лучше</div>
                <div style="font-size:13px;">«${ex.good}»</div>
              </div>
            </div>
            <div style="font-size:12px;color:var(--text-secondary);margin-top:8px;padding-left:4px;">💡 ${ex.note}</div>
          </div>
        `).join('');
      }).join('')}
    </div>
