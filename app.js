  ...
  document.getElementById('real-conclusions').innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      ${belowNorm.slice(0, 3).map(c => {
        ...
        const goodExamples = {
          'Контакт': { good: 'Здравствуйте, меня зовут [Имя], компания Дом.ру, слушаю вас', note: 'Нет представления, имени, компании — клиент не понимает, с кем говорит' },
          'Потребности': { good: 'Скажите, сколько устройств подключаете? Что важнее — скорость или стабильность? Смотрите ли ТВ?', note: 'Оператор сразу переходит к решению, не выяснив потребности' },
          'Презентация': { good: 'Учитывая, что у вас 3 устройства — подойдёт тариф «Кино и развлечения»: 300 Мбит, 200 каналов, 599 руб./мес.', note: 'Нет привязки к потребности, нет конкретных выгод и цифр' },
          'Возражения': { good: 'Понимаю вашу ситуацию. Давайте разберёмся — что именно не устраивает? Я предложу решение', note: 'Оператор игнорирует возражение вместо работы с ним' },
          'Завершение': { good: 'Итак, мы решили вопрос по тарифу. Заявка оформлена, мастер приедет 15-го. Есть ещё вопросы?', note: 'Нет подведения итогов, нет фиксации договорённостей' },
          'Модули': { good: 'Переведу на отдел оформления. Коллега видит заявку — повторно объяснять не придётся. Не кладите трубку', note: 'Нет объяснения зачем переводят, клиент теряет доверие' },
          'Слушание': { good: 'Правильно ли я понимаю — у вас не работает интернет с утра и вы уже перезагружали роутер?', note: 'Междометия — не слушание. Нужно перефразировать слова клиента' },
          'Тон': { good: 'Добрый день! Подскажите, пожалуйста, чем могу помочь?', note: 'Разговорная речь и сленг — нарушение стандарта' },
          'Активность': { good: 'Пока загружается информация — подскажите, довольны ли скоростью интернета?', note: 'Паузы — упущенное время. Оператор должен вести диалог' },
          'Грамотность': { good: 'Сейчас оформлю подключение. Услуга будет активирована в течение часа', note: 'Слова-паразиты подрывают доверие к компетентности' },
        };
        ...
        const phrasePos = (c.name === 'Завершение' || c.name === 'Модули') ? 'end' : 'start';

        return worstCalls.map(wc => `
          <div style="padding:14px 16px;background:var(--bg-card);border:1px solid var(--border-light);border-radius:10px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
              <div style="font-weight:600;font-size:13px;color:var(--text-secondary);">${c.name} · оценка <span style="color:#ef4444;font-weight:700;">${wc.scores[c.key].toFixed(1)}</span></div>
              <a href="#" onclick="event.preventDefault();document.getElementById('real-call-card').style.display='block';renderRealCallDetail(REAL_CALLS.find(x=>x.id===${wc.id}));document.getElementById('real-call-card').scrollIntoView({behavior:'smooth'});" style="font-size:12px;color:var(--primary);text-decoration:none;display:flex;align-items:center;gap:4px;">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                ${wc.operator} · ${wc.direction} · ${wc.line}
              </a>
            </div>
            <div style="display:grid;grid-template-columns:1fr auto 1fr;gap:10px;align-items:start;">
              <div style="padding:10px 12px;background:#fef2f2;border-left:3px solid #ef4444;border-radius:0 8px 8px 0;">
                ...
                <div style="font-size:13px;font-style:italic;">«${extractOpPhrase(wc.dialog, phrasePos)}»</div>
              </div>
              ...
              <div style="padding:10px 12px;background:#f0fdf4;border-left:3px solid #10b981;border-radius:0 8px 8px 0;">
                ...
                <div style="font-size:13px;">«${ref.good}»</div>
              </div>
            </div>
            <div style="font-size:12px;color:var(--text-secondary);margin-top:8px;padding-left:4px;">💡 ${ref.note}</div>
          </div>
        `).join('');
      }).join('')}
    </div>
